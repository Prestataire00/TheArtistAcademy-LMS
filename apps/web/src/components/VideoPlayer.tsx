...
export function VideoPlayer({ uaId, completionThreshold = 99 }: VideoPlayerProps) {
  ...
  if (error) {
    return (
      <div className="aspect-video bg-dark sm:rounded-lg flex items-center justify-center">
  ...
  return (
    <div className="space-y-3">
      <div className="relative aspect-video bg-black sm:rounded-lg overflow-hidden">
        {/* Loading overlay */}
        {loading && (
      ...
      {/* Completion banner */}
      {completed && (
        <div className="flex items-center gap-2 bg-green-50 border border-green-200 text-green-800 rounded-lg mx-4 sm:mx-0 px-4 py-2.5 text-sm">
          <svg className="w-5 h-5 text-green-600 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <span className="font-medium">Vidéo terminée</span>
        </div>
      )}
    </div>
  );
